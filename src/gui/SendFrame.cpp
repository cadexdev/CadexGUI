...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("CADEX://", Qt::CaseInsensitive))
    {
       _request.replace(0, 7, "CADEX:");
    }
    if(!_request.startsWith("CADEX:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with CADEX:"), QtCriticalMsg));
      return;
    }
