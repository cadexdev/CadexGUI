...
// Copyright (c) 2016-2017 The CADEX developers
...
int main(int argc, char* argv[]) {
	...
	//as root
	args << "-c" << "printf '[Desktop Entry]\\nName = CADEX URL Handler\\nGenericName = CADEX\\nComment = Handle URL Sheme cadex://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/cadex;\\nIcon = CADEX-Wallet' | tee /usr/share/applications/cadex-handler.desktop";
...
#ifdef _WIN32
	Updater d;
	if (!QCoreApplication::applicationFilePath().toLower().contains("cadex/cadex-wallet")) {
		d.checkForUpdate();
	}
#else
	Updater d;
